...
export function Stage({
    ...
    return (
        <div className="relative w-full min-h-[calc(100vh-61px)] overflow-hidden bg-[color:var(--color-ink-deeper)]">
            ...
            {/* Portrait — hard edge at the viewport edge so the character feels partly off-stage. */}
            {portrait && (
                <motion.div
                    key={`${sceneKey}-portrait`}
                    className="hidden md:block absolute bottom-0 left-0 w-[28vw] max-w-[360px] pointer-events-none"
                    initial={{ opacity: 0, y: 32, filter: "blur(4px)" }}
                    animate={{ opacity: 1, y: 0, filter: "blur(0px)" }}
                    transition={{ duration: 0.8, delay: 0.3, ease: "easeOut" }}
                >
                    {/* Soft edge feather so the portrait bleeds into the scene. */}
                    <div
                        style={{
                            maskImage:
                                "linear-gradient(to top, rgba(0,0,0,0.85), black 25%, black)",
                            WebkitMaskImage:
                                "linear-gradient(to top, rgba(0,0,0,0.85), black 25%, black)",
                        }}
                    >
            ...
            {/* Dialogue dock — asymmetric, right-leaning. */}
            <motion.div
                key={`${sceneKey}-dock`}
                className="absolute inset-x-0 bottom-0 px-4 sm:px-10 lg:px-16 pb-8 pt-20"
                initial={{ opacity: 0, y: 24 }}
                animate={{ opacity: 1, y: 0 }}
                transition={{ duration: 0.55, delay: 0.45, ease: [0.2, 0.8, 0.2, 1] }}
            >
                <div className="max-w-4xl md:ml-[22vw] lg:ml-[24vw]">
                    {children}
                </div>
            </motion.div>
